...
| Test `SKIP LOCKED` yang memeriksa hasil SETELAH semua worker selesai | Lulus untuk `FOR UPDATE` **tanpa** `SKIP LOCKED`. Worker kedua memang tetap maju — ia menunggu kunci worker pertama dilepas, lalu PostgreSQL memeriksa ulang baris yang sudah diproses, melewatinya, dan melanjutkan ke baris berikutnya. Nol pemrosesan ganda, jadi test "tidak memproses event yang sama" hijau — padahal dua instance itu **berurutan, bukan paralel**, dan acceptance criteria `Q-03` meminta yang kedua | Ukur kemajuan worker kedua **selagi yang pertama masih memegang kuncinya** (handler ditahan gerbang), bukan sesudahnya. Lalu cabut `SKIP LOCKED` saja — bukan seluruh kuncinya — dan pastikan test merah |
...
| Memakai `X-Forwarded-For` sebagai identitas rate limit | Header itu **dikirim klien**. Tanpa reverse proxy yang menimpanya — dan repo ini belum punya proxy sama sekali — penyerang cukup mengirim nilai acak tiap request untuk mendapat ember baru setiap kali, dan batas 10/menit di `/auth/*` yang menjaga brute force login lewat begitu saja. Versi pertama `R-03` melakukannya persis, **dan** mengambil entri paling KIRI — yang tetap milik klien bahkan setelah proxy berdiri, karena proxy hanya MENAMBAHKAN alamat yang dilihatnya di ujung kanan. Jadi pembatas yang sudah dibuktikan merah lewat sabotase tetap ornamental untuk lalu lintas tak terautentikasi. Test-nya sendiri memakai `X-Forwarded-For` untuk memisahkan ember per test — spoofability-nya terlihat di depan mata dan terbaca sebagai fitur | Header diabaikan TOTAL kecuali `TRUST_PROXY_HOPS > 0`, dan saat dipercaya dihitung dari **KANAN** sebanyak hop. Bawaannya `0`. Penjaganya dua test: satu membuktikan dua XFF berbeda dari satu soket berbagi SATU ember saat hop=0, satu lagi membuktikan entri kiri diabaikan saat hop=1 |
| Provider worker yang berdiri sempurna tapi **tidak pernah disuruh mulai** | `worker.module.spec.ts` membuktikan modulnya bisa dirakit dan providernya bisa diambil — itu menangkap isu #86 (gagal boot total) dan **tidak** menangkap kelas berikutnya. `AiDispatchService` (`AI-06`) punya `start()` yang teruji dan tidak dipanggil siapa pun: `bootstrapWorker()` hanya membangun konteks aplikasi, jadi antrean `ai` berdiri tanpa satu pun konsumen dan setiap `ai_jobs` mengendap `queued` **selamanya, tanpa galat**. Test integrasinya lulus karena ia membuat `Worker` BullMQ-nya sendiri. Pola "hijau tapi rusak saat dijalankan" yang KEEMPAT di repo ini, dan yang pertama ditemukan satu hari setelah kodenya ditulis | Konsumen yang harus hidup menyalakan dirinya lewat `onApplicationBootstrap`, bukan lewat baris di `main.ts` yang bisa lupa ditulis. Penjaganya `worker-boot.integration.spec.ts`: ia mem-`init()` `WorkerModule` (bukan `compile()` saja — hook lifecycle hanya jalan di `init()`), mengantrekan job, dan **tidak membuat Worker sendiri**. Ia juga memastikan `close()` benar-benar menghentikannya, supaya tidak ada worker yang selamat dari shutdown dan menumpuk satu per restart |
...
| `git stash` dikira memindahkan SEMUA pekerjaan saat memecah PR | Ia **tidak menyertakan berkas yang belum di-track**. Terjadi saat memisahkan PR PRD-saja (`INTERNAL_ERROR`, #135) dari PR kodenya: lima berkas baru tetap tinggal di working tree, `git add -A` menyapunya, dan PR yang seharusnya "nol perubahan kode" berisi 408 baris TypeScript. Repo ini memecah PR justru supaya urutannya bisa diperiksa — PRD dulu, kode belakangan — dan gabungan itu menghapus seluruh gunanya. CI tetap hijau, jadi tidak ada yang menangkapnya | `git stash -u`, atau periksa `git show --stat HEAD` **sebelum** push. Untuk PR yang klaimnya "hanya dokumen", klaim itu sendiri yang wajib diverifikasi: `gh pr view <n> --json files` |
| `{ error: … }` dikira sudah berarti bentuk §10.1 | Exception bawaan Nest memakai `{ statusCode, message, error: 'Not Found' }` — `error`-nya **string**, bukan objek ber-`code`. Pemeriksaan `'error' in isi` meloloskannya apa adanya, jadi setiap 404/403 dari framework keluar dalam bentuk Nest sementara 500 keluar dalam bentuk §10.1: satu API, dua bentuk galat, dan klien yang mencabang pada `error.code` mendapat `undefined` pada separuhnya | Periksa BENTUKNYA, bukan keberadaan kuncinya: `error` harus objek dengan `code` bertipe string. Diuji untuk 404 router DAN rute yang tidak ada sama sekali — 500 saja tidak menangkapnya |
| Menyebut gerbang hijau berdasarkan run yang DIMULAI sebelum perubahan terakhir | Suite integrasi berjalan 70–90 detik di latar; perubahan yang dibuat sementara ia berjalan **tidak ada di dalamnya**. Terjadi di PR #130: satu fixture diubah saat suite sedang jalan, hasilnya tetap dibaca sebagai bukti, dan gerbang berikutnya (lint/typecheck/unit/build) tidak memuat integrasi — jadi tidak ada yang menangkapnya sampai CI. Versi lain dari jebakan yang sama: menjalankan `pnpm test` dan `pnpm test:integration` **bersamaan** di database dev yang sama; suite integrasi mem-`TRUNCATE users` dan spec lain kehilangan barisnya di tengah jalan, yang terlihat persis seperti kegagalan test sungguhan | Run yang jadi bukti harus DIMULAI setelah perubahan terakhir. Dan hanya satu suite yang menyentuh database dev boleh berjalan pada satu waktu |
| `process.env['X'] ?? bawaan` dikira menangkap nilai kosong | `??` hanya menangkap `undefined`/`null`. Untuk `APP_URL` itu bukan kekeliruan kecil: paket `cors` membaca origin **falsy sebagai `*`**, jadi `APP_URL=` yang di-set tapi kosong menghasilkan `Access-Control-Allow-Origin: *` di REST **dan** WebSocket sekaligus, tanpa satu galat pun. Dan template env produksi di PRD §17 memang mengirim `APP_URL=` kosong dengan contohnya di komentar, jadi siapa pun yang menyalinnya untuk deploy mendapatkannya. Ekspresi yang sama tersalin di TIGA berkas, jadi lubangnya ikut tersalin | Env yang menentukan keamanan dibaca lewat SATU fungsi yang memperlakukan kosong sebagai tidak disetel, dan **gagal tertutup** (jatuh ke origin dev — di produksi itu berarti frontend ditolak CORS: keras dan terlihat). Lihat `apps/api/src/common/app-origin.ts` |
| Mengira rate limit "sudah ada dari Better-Auth" | Ada, dan bukan yang dijanjikan PRD: **mati** kecuali `NODE_ENV=production`, **600 req/menit** (100 per 10 detik — lima kali lebih longgar dari 120/menit yang dijanjikan §16.1), storage **`memory`** sehingga counter tidak dibagi antar-instance dan hilang tiap deploy, dan hanya menutup rute Better-Auth — seluruh endpoint bisnis terbuka penuh. Kunci `rl:` yang didaftarkan PRD §9.4 tidak pernah ditulis siapa pun. Temuan `R-03` | Pembatas milik sendiri di Redis (`RateLimitInterceptor`). **Interceptor, bukan guard**: guard global berjalan sebelum `SessionGuard` jadi belum tahu penggunanya, dan middleware yang me-resolve sesi sendiri menambah satu query Postgres per request — penguat serangan, bukan pembatas. Saat Redis mati ia MEMBIARKAN LEWAT (aturan keras 7) |
| Menahan kunci baris lalu memastikan transaksi lain menunggu, dan menyebutnya bukti `SELECT … FOR UPDATE` | Transaksi yang nantinya meng-`UPDATE` baris itu **mengambil kunci yang sama**, hanya lebih lambat. Jadi test "ia menunggu" tetap HIJAU setelah `.forUpdate()` dicabut — yang diuji ternyata hanya "ada sesuatu di transaksi ini yang mengunci barisnya", dan itu selalu benar. Terjadi di `P-03`: komentarnya menuliskan klaim yang salah dengan percaya diri, dan yang membantahnya cuma sabotase, bukan pembacaan ulang | Kalau yang dijaga `FOR UPDATE`, penjaganya adalah test **dua penulis bersamaan**: tanpa kunci pembacaan, keduanya membaca keadaan lama lalu satu menabrak unique index — bedanya di BENTUK KEGAGALAN (satu request 500 lalu dikirim ulang), bukan di angkanya. Ukur dulu penjaga mana yang memerahkan test mana; jangan menulis pembagian tugasnya dari dugaan |
