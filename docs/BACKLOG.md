...
| `F-05` | Deploy staging otomatis dari main (web, api, ai, … | A | W1 | 1,5 | `review` | #149 | 2026-09-25 | Workflow CD di #149. Image api diperbaiki #150. **V1–V13 dijalankan 25 Sep (#164): SEMBILAN terbukti** (V1–V6 health/CORS/header/bentuk galat, V9 webhook ditolak-tapi-hampa, V10 login bertahan muat ulang di Chrome, V12 WSS+canRead menjawab). Tetap `review` karena TIGA menahan, dan tidak satu pun soal kode: V7 menunggu #152 ter-deploy (run-nya dibatalkan diam-diam — #165), V11 terblokir karena staging tidak punya konten (workflow tidak menyemai), V13 rollback butuh akses Railway. Dua cacat ditemukan JUSTRU karena V dijalankan: `/admin` memulangkan pengguna yang sudah masuk (#154) dan sign-out lewat Bearer tidak mencabut apa pun. Tiga env yang gagal TERBUKA ditutup #156. Sembilan temuan dipecah jadi isu #154–#163, plus #165. DoD di CLAUDE.md tidak dikembalikan — itu pengumuman Dev A. |
...
| `R-04` | Observability: log terstruktur, error tracking, a… | A | W8 | 1 | `done` | #136 | 2026-09-23 | TUJUH dari sembilan metrik §17.2 dihitung dari data kita sendiri; dua sisanya (`error_rate_5xx`, `p95_latency_hub`) butuh agregasi log — dan log itu SENGAJA tidak disimpan di Postgres, karena menulis satu baris per request ke database transaksional adalah cara memastikan database mati lebih dulu daripada yang dipantaunya. Keduanya dilaporkan `butuh_agregator`, BUKAN `ok`: metrik yang melaporkan sehat karena tidak punya datanya dipercaya justru saat insiden. Agregatornya bagian `F-05`. Ambang biaya LLM: **rasio saja, tanpa absolut** (keputusan Fatih, 23 Sep). Konsekuensi yang diterima sadar dan ditulis di kode + runbook: biaya yang naik 10% sehari TIDAK akan pernah memicu alert; yang menahannya `last_7_days` di SA-03 yang membuat trennya terlihat. Error tracking: antarmuka `PelacakGalat` saja, tanpa SDK Sentry (keputusan yang sama) — Sentry tidak ada di §12. Sekalian menutup temuan **T-2 audit R-03**: tidak ada exception filter sama sekali, jadi 500 memakai bentuk bawaan Nest tanpa `code`. `INTERNAL_ERROR` masuk §10.2 lewat #135 LEBIH DULU. Versi pertama filter memeriksa `'error' in isi` — exception Nest memakai `error` sebagai STRING, jadi 404/403 framework lolos dalam bentuk Nest sementara 500 dalam bentuk §10.1; test 500 hijau, test 404 yang memerahkannya. Pemindai kode error #109 DIPERLUAS (bukan dilonggarkan) mengenali kode yang dipilih dari tabel, seluruh nilainya dihitung terpakai — dibuktikan menangkap kode karangan yang diselipkan. Tiga runbook §17.3 di `docs/runbooks/`, masing-masing menyebut apa yang BELUM ada supaya tidak dikira ada: penjadwal, agregator log, feature flag, staging. Lima sabotase merah. |
...
> | `GET /reviews/mine` | review yang saya **terima** (isu #100) |
